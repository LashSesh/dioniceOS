...
use bridge::{CognitiveSimulator, TrajectoryObserver, SpectralAnalyzer, ParameterTuner};
use core_5d::*;
...
fn main() {
    ...
    println!("Final state: Prey={:.4}, Predators={:.4}", 
    ...
    println!("  Predator spectral centroid: {:.4}", baseline_centroids[1].1);
    ...
    while current_time < 50.0 {
        ...
        if current_time - last_tune_time >= tune_interval && sim.observer().history().len() > 20 {
            ...
            println!("  Tuning at t={:.1}, QDASH resonance={:.4}, adjustments applied", 
                current_time, tuner.resonance());
        }
    ...
    println!("  Predator spectral centroid: {:.4}", cognitive_centroids[1].1);
    ...
    for &t in &[0.0, 10.0, 20.0, 30.0, 40.0, 50.0] {
        if let Some((_, state)) = cognitive_trajectory.iter()
    ...
    println!("✓ Demonstrated adaptive feedback loop ({} tuning cycles)", tune_count);
    println!("✓ System self-tuned parameters based on dynamics");
    println!("\nSpectral changes:");
    println!("  Entropy change: {:.4} → {:.4} (Δ={:.4})", 
        baseline_entropy, cognitive_entropy, cognitive_entropy - baseline_entropy);
