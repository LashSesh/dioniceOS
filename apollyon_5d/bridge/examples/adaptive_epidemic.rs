...
use bridge::{AdaptiveCoupling, TrajectoryObserver, CognitiveSimulator};
use core_5d::*;
...
fn main() {
    ...
    for t in (0..=10).map(|i| i as f64) {
        ...
        let modulation = if base_s_to_i != 0.0 { s_to_i / base_s_to_i } else { 0.0 };
    ...
    println!("Initial state: S={:.4}, I={:.4}, R={:.4}", 
        initial.get(0), initial.get(1), initial.get(2));
    println!("Final state (t=50): S={:.4}, I={:.4}, R={:.4}", 
        final_standard.get(0), final_standard.get(1), final_standard.get(2));
    println!("Peak infected: {:.4}", 
        trajectory_standard.iter().map(|(_, s)| s.get(1)).fold(0.0f64, f64::max));
    ...
    println!("Initial state: S={:.4}, I={:.4}, R={:.4}", 
        initial.get(0), initial.get(1), initial.get(2));
    println!("Final state (t=50): S={:.4}, I={:.4}, R={:.4}", 
        final_cognitive.get(0), final_cognitive.get(1), final_cognitive.get(2));
    println!("Peak infected: {:.4}", 
        trajectory_cognitive.iter().map(|(_, s)| s.get(1)).fold(0.0f64, f64::max));
    ...
    println!("  Peak infected reduction: {:.2}%", 
        100.0 * (1.0 - trajectory_cognitive.iter().map(|(_, s)| s.get(1)).fold(0.0f64, f64::max) / 
                       trajectory_standard.iter().map(|(_, s)| s.get(1)).fold(0.0f64, f64::max)));
    println!("  Final recovered difference: {:.4}", 
    ...
    for &t in &[0.0, 5.0, 10.0, 20.0, 30.0, 40.0, 50.0] {
        if let Some((_, state)) = trajectory_cognitive.iter()
            .find(|(time, _)| (*time - t).abs() < 0.15) {
            println!("{:.1}\t\t{:.4}\t\t{:.4}\t\t{:.4}", 
                t, state.get(0), state.get(1), state.get(2));
        }
    }
