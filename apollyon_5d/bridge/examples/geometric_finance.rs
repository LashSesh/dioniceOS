...
use bridge::{CognitiveSimulator, TrajectoryObserver, GeometricStateSpace};
use core_5d::*;
...
fn main() {
    ...
    println!("Initial asset values: {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
        initial.get(0), initial.get(1), initial.get(2), initial.get(3), initial.get(4));
    println!("Final asset values: {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
    ...
    println!("\nInitial geometric projection (first 5 coords): {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
        initial_geometry[0], initial_geometry[1], initial_geometry[2],
        initial_geometry[3], initial_geometry[4]);
    ...
    println!("\nGeometric trajectory length: {} steps", geo_trajectory.len());
    println!("Final asset values: {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
    ...
    println!("  Asset values: {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
        rotated.get(0), rotated.get(1), rotated.get(2), rotated.get(3), rotated.get(4));
    
    let reflected = geo_space.apply_reflection(&final_state);
    println!("\nAfter D6 reflection:");
    println!("  Asset values: {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
        reflected.get(0), reflected.get(1), reflected.get(2), reflected.get(3), reflected.get(4));
    ...
    for &t in &[0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0] {
        if let Some((_, state)) = geo_trajectory.iter()
            ...
            println!("{:.1}\t\t{:.4}\t\t{:.4}\t\t{:.4}",
                t, state.get(0), state.get(1), geom_norm);
        }
    }
    ...
    println!("States passing validation: {}/{} ({:.1}%)",
        validation_passes, geo_trajectory.len(),
    ...
    println!("\nTest state before enforcement: {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
        test_state.get(0), test_state.get(1), test_state.get(2),
        test_state.get(3), test_state.get(4));
    
    geo_space.enforce_constraints(&mut test_state);
    println!("Test state after enforcement: {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
        test_state.get(0), test_state.get(1), test_state.get(2),
        test_state.get(3), test_state.get(4));
